...
/**
 * Translated to Java by jjYBdx4IL.
 * Original source: http://blogs.zynaptiq.com/bernsee/repo/smbPitchShift.cpp
 */
...
public class SMBPitchShift {
    ...
    private double freqPerBin, expct;
    private int i,k, qpd, index, inFifoLatency, stepSize, fftFrameSize2;

    public SMBPitchShift() {
    }
    
    public void smbPitchShift(float pitchShift, int numSampsToProcess, int fftFrameSize, int osamp, int sampleRate, float[] indata, float[] outdata)
/*
...
*/
{
	...
	for (i = 0; i < numSampsToProcess; i++){
            ...
            if (gRover >= fftFrameSize) {
                ...
                Arrays.fill(gSynMagn, 0f);
                //memset(gSynFreq, 0, fftFrameSize*sizeof(float));
                Arrays.fill(gSynFreq, 0f);
                for (k = 0; k <= fftFrameSize2; k++) { 
                ...
                for (k = fftFrameSize+2; k < 2*fftFrameSize; k++) gFFTworksp[k] = 0f;
